...
const ProcessRealesHistory = () => {
    const findRole = JSON.parse(localStorage.getItem('userData'))
    ...
    const getStandardProcessRelease = () => {
        ...
        requestApiData.getProcessRelease(payload)
            .then(res => {
                setProcessRelease(res.data)
            })
            .catch(err => {
                if (err.response?.data?.status === 401) {
                    toast.error(err.response?.data?.statusText)
    ...
    const table11 = checkedValues[0]?.processPage && checkedValues[0]?.processPage.map((item) => {
        return ReactDOMServer.renderToString(
            <Table className='main_table' sx={{ marginTop: '1rem' }}>
                <TableBody >
                    <TableRow>
    ...
    const table22 = checkedValues[1]?.processPage && checkedValues[1]?.processPage.map((item) => {
        return ReactDOMServer.renderToString(
            <table>
                <tbody>
                    <tr>
    ...
    return (
        <Box>
            {findRole?.role === 'admin' ? <AdminHeader /> : <BasicHeader />}
            <Sidebar setDrawerOpen={setDrawerOpen} drawerOpen={drawerOpen} />
            <Box marginLeft={drawerOpen ? '300px' : '60px'} paddingTop='100px'>
                <Box paddingX='2rem' marginTop='1rem' display='flex' justifyContent='space-between' alignItems='center'>
                    <Box>
                                    ...
                                    {ProcessRelease.map((item, index) => {
                                        return (
                                            <TableRow
                                                ...
                                                key={item._id}
                                            >
                                                <TableCell className='talel_color' style={{ color: 'black' }}></TableCell>
                                                <TableCell className='talel_color' style={{ color: 'black' }}>
                                                    <Checkbox
                                                        color='primary'
                                                        name='checkbox'
